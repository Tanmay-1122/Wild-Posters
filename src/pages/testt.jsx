...
const STYLES = `
  ...
  .wp-chip:hover { border-color: #0a0a0a; color: #0a0a0a; transform: translateY(-1px); }
  .wp-chip.active { background: #0a0a0a; border-color: #0a0a0a; color: #fff; }
  ...
  .wp-toast {
    ...
    gap: 10px;
    white-space: nowrap;
    animation: wp-toast-in .3s cubic-bezier(.22,.68,0,1.2) both;
    pointer-events: none;
...
function ProductCard({ product: p, idx, wishlisted, onAddToCart, onWishlist, onQuickView }) {
    ...
    const discountPct = hasDiscount ? Math.round((1 - p.price / p.compareAt) * 100) : 0;
    ...
    return (
        ...
        >
            ...
            <div style={{ position: 'relative', overflow: 'hidden', background: '#f0f0ec', aspectRatio: '4/5' }}>
                <img
                    src={p.primaryImage || p.image || p.img}
                    alt={p.title || p.name}
                ...
                <div style={{ position: 'absolute', top: 10, left: 10, display: 'flex', flexDirection: 'column', gap: 4, zIndex: 3 }}>
                    ...
                    {p.badge === 'sale' && !hasDiscount && <span className="wp-badge-off">SALE</span>}
                </div>
                ...
                >
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                        <path
                            d="M12 21s-7-4.6-9.3-9C1 8.7 3.2 6 6.3 6c1.8 0 3.2 1 3.7 1.7.5-.7 1.9-1.7 3.7-1.7C16.8 6 19 8.7 21.3 12c-2.3 4.4-9.3 9-9.3 9Z"
                            stroke={wishlisted ? '#e11d48' : '#666'}
                            strokeWidth="1.8"
            ...
            <div style={{ padding: '14px 14px 16px', background: '#fff' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
                    ...
                    <div style={{ flexShrink: 0, textAlign: 'right' }}>
                        ...
                        {hasDiscount && (
                            <div style={{ fontSize: 11, color: '#aaa', textDecoration: 'line-through', lineHeight: 1 }}>{money(p.compareAt)}</div>
                        )}
                    </div>
                </div>

                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                    <span className="wp-tag">{p.tag || p.category || p.collection}</span>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000 }} className="wp-anim-fade-in">
            ...
            >
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '1px solid #e8e8e4' }}>
                    <div>
                        <div style={{ fontSize: 15, fontWeight: 900, letterSpacing: '-.02em', textTransform: 'uppercase', color: '#0a0a0a' }}>{p.title || p.name}</div>
                        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: '#999', marginTop: 2 }}>
                            {p.tag || p.category || p.collection} · SKU {p.id}
                        </div>
                    </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', overflow: 'auto', flex: 1 }}>
                    ...
                    <div style={{ flex: '1 1 260px', padding: '24px', display: 'flex', flexDirection: 'column', gap: 16, background: '#fff' }}>
                        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                            <div>
                                ...
                                <div style={{ marginTop: 6 }}>
                                    {isOut
                                        ? <span className="wp-badge-out">Out of stock</span>
                                        : <span className="wp-badge-in">In stock</span>
                                    }
                                </div>
                            </div>
                        ...
                        {p.desc && (
                            <p style={{ margin: 0, fontSize: 13, color: '#666', lineHeight: 1.65 }}>{p.desc}</p>
                        )}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                            <div style={{ background: '#f4f4f2', border: '1px solid #e0e0dc', padding: '10px 12px' }}>
                                ...
                                <div style={{ fontSize: 13, fontWeight: 700, color: '#0a0a0a' }}>{p.tag || p.category || p.collection || '—'}</div>
                            </div>
                            <div style={{ background: '#f4f4f2', border: '1px solid #e0e0dc', padding: '10px 12px' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                            {[p.tag || p.category || p.collection, 'Matte', 'A4', 'A3'].filter(Boolean).map((t, i) => (
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8, marginTop: 'auto' }}>
                            ...
                            <button
                                className={`wp-btn${wishlisted ? ' active' : ''}`}
                                onClick={() => onWishlist(p)}
                                style={{ padding: '13px 18px', minWidth: 44 }}
                                title="Wishlist"
                            >
                                <svg width="17" height="17" viewBox="0 0 24 24" fill="none">
                                    <path
                                        d="M12 21s-7-4.6-9.3-9C1 8.7 3.2 6 6.3 6c1.8 0 3.2 1 3.7 1.7.5-.7 1.9-1.7 3.7-1.7C16.8 6 19 8.7 21.3 12c-2.3 4.4-9.3 9-9.3 9Z"
                                        stroke={wishlisted ? '#e11d48' : '#0a0a0a'}
            ...
            <aside className="wp-anim-drawer" style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #e8e8e4' }}>
                    <div>
                        ...
                        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: '#999', marginTop: 2 }}>
                            {cart.reduce((s, i) => s + (i.quantity || i.qty || 1), 0)} items
                        </div>
                    </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
                    {cart.length === 0 ? (
                        <div style={{ textAlign: 'center', paddingTop: 48 }}>
                            <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: '#666', marginBottom: 8 }}>
                                Your cart is empty
                            </div>
                        ...
                        cart.map((item, idx) => (
                            <div key={idx} style={{ display: 'flex', gap: 12, marginBottom: 16, paddingBottom: 16, borderBottom: '1px solid #f0f0ec' }}>
                                <img src={item.primaryImage || item.image || item.img} alt={item.title || item.name}
                                    style={{ width: 56, height: 70, objectFit: 'cover', background: '#f0f0ec', border: '1px solid #e0e0dc', flexShrink: 0 }} />
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '-.01em', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title || item.name}</div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                            ...
                                            <input className="wp-qty-input" value={item.quantity || item.qty || 1} readOnly style={{ width: 36 }} />
                                            <button onClick={() => onSetQty(item)} className="wp-btn" style={{ padding: '3px 9px', fontSize: 14, lineHeight: 1 }}>+</button>
                                        </div>
                                        <div style={{ fontSize: 13, fontWeight: 900, color: '#0a0a0a' }}>{money(item.price * (item.quantity || item.qty || 1))}</div>
                                    </div>
                                </div>
                            </div>
                        ))
                ...
                {cart.length > 0 && (
                    <div style={{ padding: '16px 20px', borderTop: '1px solid #e8e8e4' }}>
                        ...
                        <button className="wp-btn-primary" style={{ width: '100%', padding: '14px 0', marginBottom: 8 }}>
                            Checkout
                        </button>
                        <button onClick={onClose} className="wp-btn" style={{ width: '100%', padding: '11px 0' }}>
                            Continue Shopping
                        </button>
...
/* ─── CONSTANTS ──────────────────────────────────────────────── */
const TAGS = ['All', 'Anime', 'Movies', 'Music', 'Sports', 'Minimal', 'Motivational', 'JDM'];
const SORTS = [
...
export default function ProductPage() {
    ...
    const handleAddToCart = useCallback((product) => {
        ...
        showToast(`${product.title || product.name} added to cart`, 'ok');
    ...
    const handleWishlist = useCallback((product) => {
        ...
        showToast(isWished ? 'Removed from wishlist' : `${product.title || product.name} wishlisted`, isWished ? 'default' : 'ok');
    }, [toggleWishlist, wishlistSet]);

    /* Products — use real data from the project */
    const products = trendingProducts;

    const filtered = products
        .filter(p => tag === 'All' || (p.tag || p.category || p.collection || '').toLowerCase().includes(tag.toLowerCase()))
        ...
        .sort((a, b) => {
            switch (sort) {
                ...
                case 'nameAsc': return (a.title || a.name || '').localeCompare(b.title || b.name || '');
                default: return 0;
            }
        });
        ...
        <div className="wp-page" style={{ paddingBottom: 80 }}>
            ...
            <div style={{
                ...
                <div style={{ maxWidth: 1280, margin: '0 auto', position: 'relative' }}>
                    <div className="wp-anim-slide-r" style={{ animationDelay: '0ms' }}>
                        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,.5)', marginBottom: 6 }}>
                            Wild Posters · Collections
                        </div>
                        <h1 style={{ margin: 0, fontSize: 'clamp(28px, 6vw, 52px)', fontWeight: 900, letterSpacing: '-.04em', lineHeight: 1, textTransform: 'uppercase', fontStyle: 'italic' }}>
                            Shop All<br />Posters
                        </h1>
                    </div>
            ...
            <div style={{ background: '#fff', borderBottom: '1px solid #e0e0dc', position: 'sticky', top: 0, zIndex: 100 }}>
                <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 20px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 0, overflowX: 'auto', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
                        <div style={{ display: 'flex', gap: 2, padding: '14px 0', flexShrink: 0 }}>
                            {TAGS.map((t, i) => (
                                <button
                                    key={t}
                                    className={`wp-chip${tag === t ? ' active' : ''}`}
                                    onClick={() => setTag(t)}
                                    style={{ animationDelay: `${i * 40}ms`, whiteSpace: 'nowrap' }}
                                >
                                    {t}
                                </button>
                            ))}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, padding: '10px 0 10px 12px' }}>
                            <select
                                value={avail}
            ...
            <div style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 16px 0' }}>
                ...
                {loading ? (
                    ...
                    <div className="wp-anim-fade-up" style={{ textAlign: 'center', padding: '80px 20px' }}>
                        ...
                        <div style={{ fontSize: 13, color: '#888', marginBottom: 24 }}>Try a different category or availability filter.</div>
                        <button className="wp-btn-primary" onClick={() => { setTag('All'); setAvail('all'); }} style={{ padding: '12px 28px' }}>
                            Clear Filters
                        </button>
                    </div>
                ) : (
                    <div
                        className="wp-grid"
                        style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(min(160px, 44vw), 1fr))' }}
                    >
                        {filtered.map((p, i) => (
                            <ProductCard
                                key={p.id}
                ...
                {!loading && filtered.length > 0 && (
                    <div className="wp-anim-fade-in" style={{ textAlign: 'center', padding: '32px 0 0', fontSize: 11, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: '#bbb' }}>
                        Showing {filtered.length} of {products.length} prints
                    </div>
                )}
            </div>
